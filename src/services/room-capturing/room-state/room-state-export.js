...
function createRoomState(
  ...
  if (points.length > 2) {
    ...
    const parsedObjects = JSON.parse(
      JSON.stringify([...objects.door, ...objects.window])
    ).map(object => {
    ...
    for (let i = 0; i < numControlPoints; i++) {
      ...
      switch (i) {
        ...
        default:
          ...
          const angle = vec2angleBetween(v1, v2);
          length = WALL_WIDTH / 2 / Math.cos(Math.PI - angle / 2);
          const n1 = vec2normal(points[i - 1], current);
          const n2 = vec2normal(current, points[i + 1]);
